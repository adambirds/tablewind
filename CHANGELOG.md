...
- **Search Functionality**: Added support for search with flexible implementation options:
  - `searchEnabled` prop: Enable internal search bar within the table header (both desktop and mobile)
  - `searchPlaceholder` prop: Customize the search input placeholder text
  - External search support: Any external component (e.g., navbar search) can add `?search=value` to URL parameters
  - Search automatically syncs with URL query parameters
  - Reset filters now also clears search parameter
  - New `SearchBar` component exported for external use
  - Debounced search input (300ms) to optimize API calls

### Changed

- Updated `DataTableProps` interface with new search-related props
- Modified filter reset functionality to include search parameter clearing
- Updated `TableActionsBarDesktop` and `TableActionsBarMobile` components to support search
